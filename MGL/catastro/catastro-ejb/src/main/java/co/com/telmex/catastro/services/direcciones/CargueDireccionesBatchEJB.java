...
import co.com.telmex.catastro.services.util.CargueDirLocBatch;
...
/**
 * Clase CargueDireccionesBatchEJB implementa CargueDireccionesBatchEJBRemote
 *
 * @author Deiver Rovira.
 * @version	1.0
 */
@Stateless(name = "cargueDireccionesBatchEJB", mappedName = "cargueDireccionesBatchEJB", description = "traducirCargarDireccionesBatch")
@Remote({CargueDireccionesBatchEJBRemote.class})
public class CargueDireccionesBatchEJB implements CargueDireccionesBatchEJBRemote {

    private static final Logger LOGGER = LogManager.getLogger(CargueDireccionesBatchEJB.class);
    
    public boolean validaTraduccion;
    public boolean validaTransaccionBD;
    public StringBuffer archivoPlano;
    public CargueDirLocBatch cargueBatch;
    ...
    public ArrayList<AddressResultBatch> addressResult;
     ...
     *
     */
    public CargueDireccionesBatchEJB() {
        cargueBatch = new CargueDirLocBatch();
        addressResult = new ArrayList<AddressResultBatch>();
    ...
    public byte[] processFileAddress(StringBuffer fileFlat, String user) throws UnsupportedEncodingException, ApplicationException {
        ...
        if ("2".equals(flagTransaction)) {
            ...
            if (traslateComplete(addressServices) == true) {
                try {
                    addressResult = (ArrayList<AddressResultBatch>) addressEJBValidator.queryAddressResultBatch(addressRequest, addressResult, addressServices, user);
    ...
    private ArrayList<AddressRequest> loadFile(StringBuffer fileFlat) {
        try {
            ...
            addressRequest = cargueBatch.cargaArchivo(fileFlat);
            addressResult = cargueBatch.getFileAddressResultLog();
    ...
    private ArrayList<AddressService> queryAddressBatchTraslate(ArrayList<AddressRequest> addressRequest) throws ApplicationException {
        try {
            ...
            LOGGER.error(msg);
            throw new ApplicationException("Error al momento de consultar la dirección en batch translate. EX000: " + ex.getMessage(), ex);
        }
    ...
    private ArrayList<AddressResultBatch> fillArrayAddresResult(ArrayList<AddressService> addresservice) throws ApplicationException {
        ...
        if (addressResult != null && addressServices != null) {
            while (i < topeArchivo && i < topeAdrServ) {
                ...
                if (addresservice.get(i).getAddress() == null || "".equals(addresservice.get(i).getAddress()) || (addresservice.get(i).getAddress()).equalsIgnoreCase("null")) {
                    ...
                    addressResult.get(i).setAddressTraslateGeodatas(addresservice.get(i).getAddress());
    ...
    private byte[] createFileValidateCSV(ArrayList<AddressResultBatch> addressResult) throws UnsupportedEncodingException {

        archivoCsv = cargueBatch.generarArchivo(addressResult);
        if (archivoCsv != null) {
            return archivoCsv;
        } else {
