...
@Entity
@Table(name = "MGL_DIVISIONAL", schema = "MGL_SCHEME")
@XmlRootElement
@NamedQueries({
    @NamedQuery(name = "Divisional.findAll", query = "SELECT d FROM Divisional d")})
...
public class Divisional implements Serializable{
    ...
    @SequenceGenerator(
            name = "DivisionalMgl.divisionalMglSq",
            sequenceName = "MGL_SCHEME.MGL_DIVISIONAL_SQ", allocationSize = 1)
    @GeneratedValue(generator = "DivisionalMgl.divisionalMglSq")
    @Column(name = "DIVISIONAL_ID", nullable = false)
    private BigDecimal divId;
    @Column(name = "NOMBRE_DIVICION")
