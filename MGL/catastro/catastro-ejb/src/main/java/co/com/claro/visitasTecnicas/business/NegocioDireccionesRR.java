 ...
 */
public class NegocioDireccionesRR {
    
    private static final Logger LOGGER = LogManager.getLogger(NegocioDireccionesRR.class);
    ...
    public String ValidarTipoDireccion(String codGeo, BigDecimal city) throws ApplicationException {
        ...
        switch (codGeo.trim().length()) {
            ...
            case LONGITUD_TIPO_HERENDAN_BOGOTA_MEDELLIN:
                if (geograficoPolitico != null) {
                    ...
                    throw new ApplicationException("La ciudad no tiene tipo de direccion, Cali-Medellin-Bogota");
                } else {
                    throw new ApplicationException("La ciudad no se encontro en Base de Datos");
                }
            case LONGITUD_TIPO_MAZANA_CASA:
                tipoDireccion = "BM";
                return TIPO_MAZANA_CASA;
            default:
                LOGGER.error("La longitud de codigo de direccion geo corresponde con establecidos, Direccion Intraducible");
                return "Intraducible";               
        }
    }
