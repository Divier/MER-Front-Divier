...
/**
 * Clase TaskLoad Implementa Runnable y TaskInit
 *
...
public class TaskLoad implements Runnable, TaskInit {
     ...
     * @param resource
     */
    public TaskLoad(InitialContext ic, int operation, String resource) {
    ...
    public void run() {
        if (LOAD_DATASOURCE == operation) {
            try {
                ...
                LOGGER.error("Error al momento de ejecutar la tarea. EX000: " + ex.getMessage(), ex);
            }
    ...
    public ResponseInit loadDataSource() throws NamingException {
        ...
        datasource = (javax.sql.DataSource) ic.lookup(resource);
    ...
    public ResponseInit loadProperties() throws IOException {
        ...
        try (FileInputStream item = new FileInputStream(resource)) {
