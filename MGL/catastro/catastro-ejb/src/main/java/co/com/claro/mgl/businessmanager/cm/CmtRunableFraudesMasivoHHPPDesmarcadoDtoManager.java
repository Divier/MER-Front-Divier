 ...
 * @author enriquedm
 */
public class CmtRunableFraudesMasivoHHPPDesmarcadoDtoManager implements Runnable {

    private static final Logger LOGGER = LogManager.getLogger(CmtRunableFraudesMasivoHHPPDesmarcadoDtoManager.class);
    ...
    List<FraudesHHPPDesmarcadoMasivoDto> listaModificacion;
    int perfil;
    ...
    public CmtRunableFraudesMasivoHHPPDesmarcadoDtoManager() {
    }

    public CmtRunableFraudesMasivoHHPPDesmarcadoDtoManager(List<FraudesHHPPDesmarcadoMasivoDto> listaModificacion, int perfil, String usuario, String nombreArchivo) {
    ...
    public void run() {
        int registros = 0;
        List<FraudesHHPPDesmarcadoMasivoDto> fmMgls = new ArrayList<FraudesHHPPDesmarcadoMasivoDto>();

        try {
            CmtFraudesMasivoHHPPDesmarcadoDtoMgl.cleanBeforeStart();
            CmtFraudesMasivoHHPPDesmarcadoDtoMgl.startProcess(usuario);
        } catch (ApplicationException ex) {
            CmtFraudesMasivoHHPPDesmarcadoDtoMgl.setMessage(ex.getMessage());
        ...
        if (registros > 0) {
            try {
                ...
                CmtFraudesMasivoHHPPDesmarcadoDtoMgl.setUserRunningProcess(this.usuario);
                CmtFraudesMasivoHHPPDesmarcadoDtoMgl.setNombreArchivo(this.nombreArchivo);
        ...
        if (registros > 0) {
            CmtFraudesMasivoHHPPDesmarcadoDtoMgl.setNumeroRegistrosAProcesar(registros);
            if (!listaModificacion.isEmpty()) {
                    try {
                        int count = 0;
                        int cantFallido = 0;
                        for (FraudesHHPPDesmarcadoMasivoDto m : this.listaModificacion) {
                            if (m.getIdDireccionDetallada() != null && !m.getIdDireccionDetallada().equals(BigDecimal.ZERO)) {
                                ...
                                if (cmtIdDireccion != null) {
                                    ...
                                    if (m.getDireccion() != null && !m.getDireccion().isEmpty()) {
                                        ...
                                        cmtDireccion = buscarDireccion(m.getDireccion(), m.getCentroPoblado(), barrio, null);
                            ...
                            count += 1;
                            CmtFraudesMasivoHHPPDesmarcadoDtoMgl.setNumeroRegistrosProcesados(count);
                        ...
                        CmtFraudesMasivoHHPPDesmarcadoDtoMgl.setListFraudesHHPPDesmarcadoMasivoDto(fmMgls);
        ...
        try {
            CmtFraudesMasivoHHPPDesmarcadoDtoMgl.endProcess(fmMgls);
        } catch (ApplicationException ex) {
            CmtFraudesMasivoHHPPDesmarcadoDtoMgl.setMessage(ex.getMessage());
            String msg = "Se produjo un error al momento de ejecutar el método '" + ClassUtils.getCurrentMethodName(this.getClass()) + "': " + ex.getMessage();
            LOGGER.error(msg, ex);
     ...
     */
    private CmtDireccionDetalladaMgl buscarDireccion(String direccion, String centroPoblado, String barrio, String tipoDireccion) {
        ...
        try {
            ...
            direccionDet = cmtDireccionDetalleMglManager.findDireccionDetalladaByDireccionTexto(direccion, idCentroPoblado.getGpoId(), true, barrio, tipoDireccion);
        } catch (ApplicationException e) {
                ...
                try {
                    ...
                    if (cmtDireccionDetalladaMgl.getHhppMgl() != null) {
                        CmtDefaultBasicResponse responseMarcas = hhppMglManager.eliminarMarcasHhpp(
                                cmtDireccionDetalladaMgl.getHhppMgl(), mapaDireccionesFraudesAdd.get(cmtDireccionDetalladaMgl));
                        if (responseMarcas.getMessage() != null && !responseMarcas.getMessage().isEmpty() 
                            ...
                            if (listaModificacion != null && !listaModificacion.isEmpty()) {
                                listaModificacion.forEach((FraudesHHPPDesmarcadoMasivoDto dto) -> {
                    ...
                    LOGGER.error("Se produjo error al realizar la actualización  Eliminación de marcas en RR " + e.getMessage());
                }
            });
        } catch (Exception e) {
    ...
    /**
     * Diana -> Agregar marca a la tabla final para enviar las marcas al servicio
     */
    public Boolean addMarca(CmtDireccionDetalladaMgl direccion, MarcasMgl marca) {
        try {
            ...
            LOGGER.error("Se produjo error al remover marcas en la lista del archivo " + e.getMessage());
            return false;
        }
        return true;
    }
     ...
     */
    public void guardarCargueArchivosLogItemBD(List<FraudesHHPPDesmarcadoMasivoDto> listaModificacion,
            long idArchivoLog) throws ApplicationException {
        for (FraudesHHPPDesmarcadoMasivoDto item : listaModificacion) {
    ...
    private String cabeceraModificadaCsv() {
        ...
        registro.append(LINEA).append(Constant.SEPARADOR).append(ID_DIRECCION_DETALLADA).append(Constant.SEPARADOR).append(DEPARTAMENTO).append(Constant.SEPARADOR).append(MUNICIPIO)
                .append(Constant.SEPARADOR).append(CENTRO_POBLADO).append(Constant.SEPARADOR).append(NODO).append(Constant.SEPARADOR).append(DIRECCION).append(Constant.SEPARADOR)
                .append(COD_MARCAS_FRAUDE).append(Constant.SEPARADOR).append(COD_MARCAS_FRAUDE_NUEVA).append(Constant.SEPARADOR).append(DETALLE);
     ...
     */
    public String DataOriginal(FraudesHHPPDesmarcadoMasivoDto listaModificacion) {
        ...
        registro.append(listaModificacion.getLinea()).append(Constant.SEPARADOR).append(idDireccionDetallada).append(Constant.SEPARADOR).append(listaModificacion.getDepartamento())
                ...
                .append(Constant.SEPARADOR).append(listaModificacion.getCodMarca()).append(Constant.SEPARADOR).append(listaModificacion.getCodMarcaNuevo());
        return registro.toString();
    }
     ...
     * @param listaModificacion
     */
    public String DataModificada(FraudesHHPPDesmarcadoMasivoDto listaModificacion) {
        String idDireccionDetallada = listaModificacion.getIdDireccionDetallada() != null ? listaModificacion.getIdDireccionDetallada().toString() : "";
