 ...
 * @author enriquedm
 */
@ManagedBean(name = "fraudesHHPPCargueMasivo")
@ViewScoped
public class FraudesHHPPCargueMasivo implements Serializable {

    private static final Logger LOGGER = LogManager.getLogger(FraudesHHPPCargueMasivo.class);
    private String nameFile;
    private String msgCargaReporte;
    private boolean showPanelCargueMarcado;
    private boolean showPanelTablaProcesos;
    private org.primefaces.model.file.UploadedFile upFile;
    ...
    private List<FraudesHHPPMasivoDto> listaModificacion = null;
    private List<FraudesHHPPMasivoDto> reporteFinal;
    ...
    List<FraudesHHPPMasivoDto> listGeneral = new ArrayList<FraudesHHPPMasivoDto>();
    
    //Opciones agregadas para Rol
    private final String BTPROARCHMAR = "BTPROARCHMAR"; 
     ...
     */
    public FraudesHHPPCargueMasivo() {
        try {
            ...
            FacesUtil.mostrarMensajeError("Error al FraudesHHPPCargueMasivo. " + e.getMessage(), e, LOGGER);
        } catch (Exception e) {
            FacesUtil.mostrarMensajeError("Error al FraudesHHPPCargueMasivo. " + e.getMessage(), e, LOGGER);
        }
    }
    ...
    public void init() {
        ...
        limpiarValores();
        if (!CmtFraudesMasivoHHPPDtoMgl.isIsProcessing()) {
            if (CmtFraudesMasivoHHPPDtoMgl.getEndProcessDate() != null && new Date().before(sumarTiempoFecha(CmtFraudesMasivoHHPPDtoMgl.getEndProcessDate(), this.minutos))) {
                numRegProcesados = CmtFraudesMasivoHHPPDtoMgl.getNumeroRegistrosProcesados();
                numRegAProcesar = CmtFraudesMasivoHHPPDtoMgl.getNumeroRegistrosAProcesar();
                nameFile = CmtFraudesMasivoHHPPDtoMgl.getNombreArchivo();
                setProgress(true);
                this.usuarioProceso = CmtFraudesMasivoHHPPDtoMgl.getUserRunningProcess();
            } else {
                limpiarValores();
                setProgress(false);
                setFinish(false);
...
//                this.refresh = REFRESH_MIN;
                if (listaModificacion != null && !listaModificacion.isEmpty()) {
                    direccionDetalleMglFacade.actualizarCargueFraudesMasivoHHPP(listaModificacion, usuario, perfil, this.nombre);
//                    this.finish = CmtFraudesMasivoHHPPDtoMgl.isIsFinished();
                }
                if (listaModificacion.size() > 0) {
                    setProgress(true);
                    this.usuarioProceso = CmtFraudesMasivoHHPPDtoMgl.getUserRunningProcess();
            ...
            FacesUtil.mostrarMensajeError("Error en cargarArchivo de FraudesHHPPCargueMasivo.  " + e.getMessage(), e, LOGGER);
        } catch (Exception e) {
            FacesUtil.mostrarMensajeError("Se genera error en FraudesHHPPCargueMasivo. " + e.getMessage(), e, LOGGER);
        }finally{
            limpiarValores();
        }
    }

    private List<FraudesHHPPMasivoDto> generarListaGeneral() throws IOException {
        String line;
        String cvsSplitBy = ";";
        File archivo = null;
        ...
        try {
            ...
            while ((line = bre.readLine()) != null) {
                FraudesHHPPMasivoDto masivo = new FraudesHHPPMasivoDto();
                if (!line.isEmpty()) {
                    if (i == 0) {
            ...
            LOGGER.info("Error en generarListaGeneral de FraudesHHPPCargueMasivo : " + e);
            if (archivo != null) {
                try {
    ...
    private Integer tiempoReporte(String nombreParametro) {
        ...
        try {
            ...
            LOGGER.error("Error tiempoMaximo de FraudesHHPPCargueMasivo " + e.getMessage());
            return DEFAULT;
        }
        return numero;
    }
     ...
     * @param reporte
     */
    private void exportarArchivoXls(List<FraudesHHPPMasivoDto> reporte) {
        ...
        try {
            if (reporte != null && !reporte.isEmpty()) {
                ...
                for (FraudesHHPPMasivoDto r : reporte) {
            ...
            FacesUtil.mostrarMensajeError("Se genera error en exportarArchivoXls class FraudesHHPPCargueMasivo " + e.getMessage(), e, LOGGER);
        }
    }
     ...
     * @param reporte
     */
    private void exportarArchivoCsv(List<FraudesHHPPMasivoDto> reporte) {
        try {
            if (reporte != null && !reporte.isEmpty()) {
                HSSFWorkbook workbook = new HSSFWorkbook();
                Map<String, Object[]> mapDataEstado = new TreeMap<String, Object[]>();
            ...
            FacesUtil.mostrarMensajeError("Se genera error en exportarArchivoCsv class FraudesHHPPCargueMasivo " + e.getMessage(), e, LOGGER);
        }
    }
    ...
    private void fillSheetbookCsv(HSSFWorkbook workbook, String sheetName, Map<String, Object[]> data) {
        try {
            ...
            FacesUtil.mostrarMensajeError("Se generó error en fillSheetbook de FraudesHHPPCargueMasivo" + e.getMessage(), e, LOGGER);
        }
    }
...
//                            for (String dato : datosMod) {
    ...
    public void limpiarValores() {
        numRegAProcesar = 0;
        CmtFraudesMasivoHHPPDtoMgl.setNombreArchivo("");
        CmtFraudesMasivoHHPPDtoMgl.setNumeroRegistrosAProcesar(0);
        CmtFraudesMasivoHHPPDtoMgl.setNumeroRegistrosProcesados(0);
        CmtFraudesMasivoHHPPDtoMgl.setUserRunningProcess("");
        CmtFraudesMasivoHHPPDtoMgl.setIsProcessing(false);
    }
    ...
    public org.primefaces.model.file.UploadedFile getUpFile() {
        return upFile;
    }

    public void setUpFile(org.primefaces.model.file.UploadedFile upFile) {
        this.upFile = upFile;
    }
    ...
    public String getUsuarioProceso() {
        usuarioProceso = CmtFraudesMasivoHHPPDtoMgl.getUserRunningProcess();
    ...
    public void refreshCantReg() {
        reporteFinal = CmtFraudesMasivoHHPPDtoMgl.getListFraudesHHPPMasivoDto();
        numRegProcesados = CmtFraudesMasivoHHPPDtoMgl.getNumeroRegistrosProcesados();
        numRegAProcesar = CmtFraudesMasivoHHPPDtoMgl.getNumeroRegistrosAProcesar();
    ...
    public boolean getReady() {
        if (CmtFraudesMasivoHHPPDtoMgl.getEndProcessDate() != null
                && !new Date().before(sumarTiempoFecha(CmtFraudesMasivoHHPPDtoMgl.getEndProcessDate(), this.minutos))) {
            limpiarValores();
            setProgress(false);
            setFinish(false);
            try {
    ...
    public boolean validarOpcionProcesarArchivo() {
        return validarEdicionRol(BTPROARCHMAR);
    }
