...
/**
 * Manejo de operaciones en Archivos de Fichas New
 * 
 * @author Miguel Barrios Hitss
 * @version 1.1 Rev Miguel Barrios Hitss
 */
public class ExportArchivosFichasNew {

    private static final Logger LOGGER = LogManager.getLogger(ExportArchivosFichas.class);
    String fileName;
    PreFichaMglNew preFichaMgl;

    private List<PreFichaTxtMglNew> edificiosVtXls;
    private List<PreFichaTxtMglNew> casasRedExternaXls;
    private List<PreFichaTxtMglNew> conjCasasVtXls;
    ...
    public ExportArchivosFichasNew(GeograficoPoliticoMglFacadeLocal geograficoPoliticoMglFacadeLocal) {
        this.geograficoPoliticoMglFacadeLocal = geograficoPoliticoMglFacadeLocal;
    }

    public ExportArchivosFichasNew(String fileName, PreFichaMglNew preFichaMgl, List<PreFichaTxtMglNew> edificiosVtXls, List<PreFichaTxtMglNew> casasRedExternaXls, List<PreFichaTxtMglNew> conjCasasVtXls, List<PreFichaXlsMglNew> hhppSNXls, HashMap<PreFichaXlsMglNew, CmtCuentaMatrizMgl> mapaCCMMCreadas, List<AuxExportFichas> mapaHHPPCreadas, List<AuxExportFichas> mapaErroresYDuplicados, GeograficoPoliticoMglFacadeLocal geograficoPoliticoMglFacadeLocal,NodoMglFacadeLocal nodoMglFacadeLocal) {
    ...
    private void crearHojaEdificiosVT(XSSFWorkbook workbook, Object[] cabecerasHist, Map<String, Object[]> datosHojaHist, Map<String, String> datosAddHist) {
        ...
        if (edificiosVtXls != null && !edificiosVtXls.isEmpty() && datosHojaHist == null) {
            for (PreFichaTxtMglNew p : edificiosVtXls) {
    ...
    private void crearHojaCasasRedExterna(XSSFWorkbook workbook, Object[] cabecerasHist, Map<String, Object[]> datosHojaHist, Map<String, String> datosAddHist) {
        ...
        if (casasRedExternaXls != null && !casasRedExternaXls.isEmpty() && datosHojaHist == null) {
            for (PreFichaTxtMglNew p : casasRedExternaXls) {
                BigDecimal totalHHPPPreficha = new BigDecimal(BigInteger.ZERO).add(p.getAptos()).add(p.getLocales()).add(p.getOficinas());
                totalHhhp = totalHhhp.add(totalHHPPPreficha);
    ...
    private void crearHojaConjuntoCasasVT(XSSFWorkbook workbook, Object[] cabecerasHist, Map<String, Object[]> datosHojaHist, Map<String, String> datosAddHist) {
        ...
        if (conjCasasVtXls != null && !conjCasasVtXls.isEmpty()) {
            for (PreFichaTxtMglNew p : conjCasasVtXls) {
                BigDecimal totalHHPPPreficha = new BigDecimal(BigInteger.ZERO).add(p.getAptos()).add(p.getLocales()).add(p.getOficinas());
                totalHhhp = totalHhhp.add(totalHHPPPreficha);
                datosHoja.put(String.valueOf(fila),
                        new Object[]{p.getId(), p.getNombreConj(),
    ...
    private void crearHojaHHPPSNFichas(XSSFWorkbook workbook, Object[] cabecerasHist, Map<String, Object[]> datosHojaHist, Map<String, String> datosAddHist) {
        ...
        if (hhppSNXls != null && !hhppSNXls.isEmpty()) {
            ...
            for (PreFichaXlsMglNew p : hhppSNXls) {
    ...
    private void crearHojaCCMMCreadas(XSSFWorkbook workbook, Object[] cabecerasHist, Map<String, Object[]> datosHojaHist, Map<String, String> datosAddHist) throws ApplicationException {
        ...
        if (mapaCCMMCreadas != null && mapaCCMMCreadas.keySet() != null && !mapaCCMMCreadas.keySet().isEmpty()) {
            for (PreFichaXlsMglNew p : mapaCCMMCreadas.keySet()) {
                ...
                GeograficoPoliticoMgl centro = geograficoPoliticoMglFacadeLocal.findById(nodo != null ? nodo.getGpoId() : dataHistorico.getIdNodo());
    ...
    private void crearHojaHHPPCreados(XSSFWorkbook workbook, Object[] cabecerasHist, Map<String, Object[]> datosHojaHist, Map<String, String> datosAddHist) throws ApplicationException {
        ...
        if (mapaHHPPCreadas != null && !mapaHHPPCreadas.isEmpty()) {
            for (AuxExportFichas aux : mapaHHPPCreadas) {
                ...
                if (aux.getPrefichaNew() == null) {
                    continue;
                }
                
                NodoMgl nodo = nodoMglFacadeLocal.findByCodigo(aux.getPrefichaNew().getNodo());
                GeograficoPoliticoMgl centro = geograficoPoliticoMglFacadeLocal.findById(nodo != null ? nodo.getGpoId() : dataHistorico.getIdNodo());
                GeograficoPoliticoMgl city = geograficoPoliticoMglFacadeLocal.findById(centro.getGeoGpoId());
                GeograficoPoliticoMgl state = geograficoPoliticoMglFacadeLocal.findById(city.getGeoGpoId());
                ...
                if (aux.getHhppMGL() != null && aux.getHhppMGL().getDireccionObj() != null) {
                    ...
                    switch (aux.getPrefichaNew().getIdTipoDireccion()) {
                    ...
                    datosHoja.put(String.valueOf(fila),
                            new Object[]{
                                ...
                                "",
                                "",
                                aux.getPrefichaNew().getBarrio(),
                                aux.getDireccionHHPP().split("\\|")[1],
                                nodo.getNodNombre(),
                                aux.getPrefichaNew().getFichaGeorreferenciaMgl() == null ? "" : aux.getPrefichaNew().getFichaGeorreferenciaMgl().getCambioEstrato(),
                                aux.getPrefichaNew().getFichaGeorreferenciaMgl() == null ? "" : aux.getPrefichaNew().getFichaGeorreferenciaMgl().getNeighborhood(),
                                aux.getPrefichaNew().getFichaGeorreferenciaMgl() == null ? "" : aux.getPrefichaNew().getFichaGeorreferenciaMgl().getLevelEconomic(),
                                aux.getPrefichaNew().getFichaGeorreferenciaMgl() == null ? "" : aux.getPrefichaNew().getFichaGeorreferenciaMgl().getAddress(),
                                aux.getPrefichaNew().getFichaGeorreferenciaMgl() == null ? "" : aux.getPrefichaNew().getFichaGeorreferenciaMgl().getSource(),
                                aux.getPrefichaNew().getFichaGeorreferenciaMgl() == null ? "" : aux.getPrefichaNew().getFichaGeorreferenciaMgl().getAlternateAddress(),
                                aux.getPrefichaNew().getFichaGeorreferenciaMgl() == null ? "" : aux.getPrefichaNew().getFichaGeorreferenciaMgl().getLocality(),
                                aux.getPrefichaNew().getFichaGeorreferenciaMgl() == null ? "" : aux.getPrefichaNew().getFichaGeorreferenciaMgl().getAppletStandar(),
                                aux.getPrefichaNew().getFichaGeorreferenciaMgl() == null ? "" : aux.getPrefichaNew().getFichaGeorreferenciaMgl().getCx(),
                                aux.getPrefichaNew().getFichaGeorreferenciaMgl() == null ? "" : aux.getPrefichaNew().getFichaGeorreferenciaMgl().getCy(),
                                "HHPP CREADO"});
                    fila++;
                }
    ...
    private void crearHojaErroresDuplicados(XSSFWorkbook workbook, Object[] cabecerasHist, Map<String, Object[]> datosHojaHist, Map<String, String> datosAddHist) throws ApplicationException {
        ...
        if (mapaErroresYDuplicados != null && !mapaErroresYDuplicados.isEmpty()) {
            for (AuxExportFichas aux : mapaErroresYDuplicados) {
                ...
                GeograficoPoliticoMgl city = geograficoPoliticoMglFacadeLocal.findById(centro.getGeoGpoId());
                GeograficoPoliticoMgl state = geograficoPoliticoMglFacadeLocal.findById(city.getGeoGpoId());
                totalHhhp = totalHhhp.add(aux.getTipoRegistro() == AuxExportFichas.TIPO_HHPP ? BigDecimal.ONE : aux.getPrefichaNew().getTotalHHPP());
                datosHoja.put(String.valueOf(fila),
                        new Object[]{
                            "",
                            "",
                            aux.getPrefichaNew().getViaPrincipal(),
                            aux.getPrefichaNew().getPlaca(),
                            state.getGpoNombre(),
                            city.getGpoNombre(),
                            centro.getGpoNombre(),
                            aux.getTipoRegistro() == AuxExportFichas.TIPO_HHPP ? BigDecimal.ONE : aux.getPrefichaNew().getTotalHHPP(),
                            aux.getTipoRegistro() == AuxExportFichas.TIPO_HHPP ? "" : aux.getPrefichaNew().getAptos(),
                            aux.getTipoRegistro() == AuxExportFichas.TIPO_HHPP ? "" : aux.getPrefichaNew().getLocales(),
                            aux.getTipoRegistro() == AuxExportFichas.TIPO_HHPP ? "" : aux.getPrefichaNew().getPisos(),
                            aux.getPrefichaNew().getClasificacion(),
                            aux.getPrefichaNew().getBarrio(),
                            aux.getTipoRegistro() == AuxExportFichas.TIPO_HHPP ? aux.getDireccionHHPP().split("\\|")[1] : aux.getPrefichaNew().getDistribucion(),
                            nodo.getNodNombre(),
                            aux.getPrefichaNew().getFichaGeorreferenciaMgl() != null ? aux.getPrefichaNew().getAmp() : "",
                            aux.getPrefichaNew().getFichaGeorreferenciaMgl() != null ? aux.getPrefichaNew().getFichaGeorreferenciaMgl().getCambioEstrato() : "",
                            aux.getPrefichaNew().getFichaGeorreferenciaMgl() != null ? aux.getPrefichaNew().getFichaGeorreferenciaMgl().getNeighborhood() : "",
                            aux.getPrefichaNew().getFichaGeorreferenciaMgl() != null ? aux.getPrefichaNew().getFichaGeorreferenciaMgl().getLevelEconomic() : "",
                            aux.getPrefichaNew().getFichaGeorreferenciaMgl() != null ? aux.getPrefichaNew().getFichaGeorreferenciaMgl().getAddress() : "",
                            ...
                            aux.getPrefichaNew().getFichaGeorreferenciaMgl() != null ? aux.getPrefichaNew().getFichaGeorreferenciaMgl().getCx() : "",
                            aux.getPrefichaNew().getFichaGeorreferenciaMgl() != null ? aux.getPrefichaNew().getFichaGeorreferenciaMgl().getCy() : "",
                            (aux.getPrefichaNew().getObservaciones() != null ? aux.getPrefichaNew().getObservaciones() : "") + " - " + (aux.getPrefichaNew().getObservacionFicha() != null ? aux.getPrefichaNew().getObservacionFicha() : "") + (aux.getDrDireccion() != null ? aux.getDrDireccion().getObservacionFicha() : "")});
                fila++;
            }
