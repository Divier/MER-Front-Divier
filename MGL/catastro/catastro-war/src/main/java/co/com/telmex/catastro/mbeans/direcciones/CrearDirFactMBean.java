...
import co.com.telmex.catastro.delegate.CrearDirFactDelegate;
...
/**
 * Clase CrearDirFactMBean Extiende de BaseMBean Implementa Serialización.
 *
 * @author Deiver Rovira
 * @version	1.0
 */
@ViewScoped
@ManagedBean(name = "CrearDirFactMBean")
public class CrearDirFactMBean extends BaseMBean implements Serializable {
    ...
    private static final Logger LOGGER = LogManager.getLogger(CrearDirFactMBean.class);

    /**
     *
     */
    public CrearDirFactMBean() {
        try {
            ...
            FacesUtil.mostrarMensajeError("Error en CrearDirFactMBean." + e.getMessage(), e, LOGGER);
        } catch (Exception e) {
            FacesUtil.mostrarMensajeError("Error en CrearDirFactMBean." + e.getMessage(), e, LOGGER);
        }
        size = 0;
        ...
        if (Archivo.length != 0 && !"".equals(Archivo.toString()) && Archivo != null) {
            ...
            for (int column = 0; column < Fila.length; column++) {
                ...
                switch (column) {
                    ...
                    case 3:
                        if ("NO. DE LA UNIDAD".equals(Campo) == false) {
                            messageValidation = MSG_ERROR_ESTRUCTURA;
                            err = true;
                        }
                        break;
                    ...
                    case 4:
                        if ("CALLE O CRA".equals(Campo) == false) {
                            messageValidation = MSG_ERROR_ESTRUCTURA;
                            err = true;
                        }
                        break;
                    ...
                    case 5:
                        if ("APARTAMENTO".equals(Campo) == false) {
                            messageValidation = MSG_ERROR_ESTRUCTURA;
                            err = true;
                        }
                        break;
                    ...
                    case 6:
                        if ("COMPLEMENTO".equals(Campo) == false) {
                            messageValidation = MSG_ERROR_ESTRUCTURA;
                            err = true;
                        }
                        break;
                    ...
                    case 8:
                        if ("DEPARTAMENTO".equals(Campo) == false) {
                            messageValidation = MSG_ERROR_ESTRUCTURA;
                            err = true;
                        }
                        break;
    ...
    public void doProcess() throws ApplicationException {
        String flag = "";
        try {
            setFileDonwload(CrearDirFactDelegate.getCrearDirFactEjb(getFileLoaded(), user.getUsuLogin()));
        } catch (Exception er) {
            FacesUtil.mostrarMensajeError("Error en doProcess." + er.getMessage() , er, LOGGER);
            try {
                flag = CrearDirFactDelegate.getFlagIndicator();
                messageError = CrearDirFactDelegate.getMessageError();
                setMessage(messageError);
            } catch (Exception e) {
                FacesUtil.mostrarMensajeError("Error en CrearDirFactDelegate. " + e.getMessage() , e, LOGGER);
            }
