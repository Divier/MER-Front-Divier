 ...
 */
@XmlRootElement
public class ResponseDataInformacionAdicionalSubEdificio {
    ...
    public ResponseDataInformacionAdicionalSubEdificio() {
    }

    public ResponseDataInformacionAdicionalSubEdificio(String responseString) {
        String[] spliter;
        spliter = responseString.split(Pattern.quote("|"));
        if (spliter.length > 0) {
            descripcionSubedificio = spliter[0];
            nombreEdificio = spliter[1];
            codigoOrigenDatos = spliter[2];
            descripcionOrigenDatos = spliter[3];
            codigoTipoProyecto = spliter[4];
            nombreTipoProyecto = spliter[5];
            codigoConstructor = spliter[6];
            nombreConstructor = spliter[7];
            fechaEntrega = spliter[8];
            codigoEstado = spliter[9];
            descripcionEstado = spliter[10];
            codigoAsesor = spliter[11];
            nombreAsesor = spliter[12];
            codigoEspecialista = spliter[13];
            nombreEspecialista = spliter[14];
            codigoSupervisor = spliter[15];
            nombreSupervisor = spliter[16];
            codigoTecnico = spliter[17];
            nombreTecnico = spliter[18];
            vt = spliter[19];
            fechaVT = spliter[20];
            costoVT = spliter[21];
            reDiseno = spliter[22];
            fechaReporteDiseno = spliter[23];
            fechaRespuestaDiseno = spliter[24];
            cierre = spliter[25];
            fechaRecibidoCierre = spliter[26];
            meta = spliter[27];
            fechaInicioEjecucion = spliter[28];
            rePlanos = spliter[29];
            fechaSolicitudPlanos = spliter[30];
            fechaEntregaPlanos = spliter[31];
            conectRCorriente = spliter[32];
            fechaSolicitudConectRCorriente = spliter[33];
            fechaEntregaConectRCorriente = spliter[34];
            codigoMotivo = spliter[35];
            descripcionMotivo = spliter[36];
            fechaFinEjecucion = spliter[37];
            costoEjecucion = spliter[38];
        }
    }
