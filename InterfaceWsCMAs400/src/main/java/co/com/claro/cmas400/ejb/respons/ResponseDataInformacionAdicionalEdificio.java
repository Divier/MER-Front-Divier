 ...
 */
@XmlRootElement
public class ResponseDataInformacionAdicionalEdificio {
...
@XmlElement
    ...
    public ResponseDataInformacionAdicionalEdificio() {
    }

    public ResponseDataInformacionAdicionalEdificio(String responseString) {
        String[] spliter;
        spliter = responseString.split(Pattern.quote("|"));
        if (spliter.length > 0) {
            descripcionEdificio = spliter[0];
            codigoOrigenDatos = spliter[1];
            descripcionOrigenDatos = spliter[2];
            codigoTipoProyecto = spliter[3];
            nombreTipoProyecto = spliter[4];
            codigoConstructor = spliter[5];
            nombreConstructor = spliter[6];
            fechaEntrega = spliter[7];
            codigoEstado = spliter[8];
            descripcionEstado = spliter[9];
            codigoAsesor = spliter[10];
            nombreAsesor = spliter[11];
            codigoEspecialista = spliter[12];
            nombreEspecialista = spliter[13];
            codigoSupervisor = spliter[14];
            nombreSupervisor = spliter[15];
            codigoTecnico = spliter[16];
            nombreTecnico = spliter[17];
            vt = spliter[18];
            fechaVT = spliter[19];
            costoVT = spliter[20];
            reDiseno = spliter[21];
            fechaReporteDiseno = spliter[22];
            fechaRespuestaDiseno = spliter[23];
            cierre = spliter[24];
            fechaRecibidoCierre = spliter[25];
            meta = spliter[26];
            fechaInicioEjecucion = spliter[27];
            rePlanos = spliter[28];
            fechaSolicitudPlanos = spliter[29];
            fechaEntregaPlanos = spliter[30];
            conectRCorriente = spliter[31];
            fechaSolicitudConectRCorriente = spliter[32];
            fechaEntregaConectRCorriente = spliter[33];
            codigoMotivo = spliter[34];
            descripcionMotivo = spliter[35];
            fechaFinEjecucion = spliter[36];
            costoEjecucion = spliter[37];
