 ...
 */
@XmlRootElement
public class ResponseDataGestionDeAvanzada {
    ...
    public ResponseDataGestionDeAvanzada() {
    }

    public ResponseDataGestionDeAvanzada(String responseString) {
        String[] spliter;
        spliter = responseString.split(Pattern.quote("|"));
        if (spliter.length > 0) {
            codigoEdificio = spliter[0];
            nombreEdificio = spliter[1];
            nombreAsesor = spliter[2];
            telefono1 = spliter[3];
            codigoSupervisor = spliter[4];
            telefono2 = spliter[5];
            descripcionEstado = spliter[6];
            estado = spliter[7];
            codigoSubEdificio = spliter[8];
            nombreSubEdificio = spliter[9];
            fechaGestion = spliter[10];
            observacion1 = spliter[11];
            observacion2 = spliter[12];
            observacion3 = spliter[13];
            observacion4 = spliter[14];
            observacion5 = spliter[15];
        }
    }
